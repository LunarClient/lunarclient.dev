...
Either `serverId` *or* `serverAddress` must be provided. If `serverId` is provided, the launcher will automatically connect to the IP defined
in [ServerMappings](https://lunarclient.dev/server-mappings/introduction). If `serverAddress` is provided, the launcher will use `serverAddress`
...
[ServerMappings](https://lunarclient.dev/server-mappings/introduction) entry, it will use the associated recommended version. Otherwise,
