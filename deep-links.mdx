...
almost anywhere else users have the ability to click on 
...
`forceRecommendedVersion` will only function if we can determine the recommended version for your server. If a `serverId` was passed in, we
use the recommended version from its [ServerMappings](https://lunarclient.dev/server-mappings/introduction) entry. If a `serverAddress` was
provided, and the launcher can associate the `serverAddress` with a [ServerMappings](https://lunarclient.dev/server-mappings/introduction)
entry, it will use the associated recommended version.
